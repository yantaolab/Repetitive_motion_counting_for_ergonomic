...
class Repetitive_pose(pl.LightningModule):
    ...
    def __init__(self, train_x, train_y, valid_x, valid_y, dim, heads, enc_layer, learning_rate, seed, num_classes, alpha):
        ...
        #self.loss = FocalLoss(alpha=1.0, gamma=2.0)
        self.num_classes = num_classes
        self.miner = miners.MultiSimilarityMiner()
        self.loss_func = losses.TripletMarginLoss()

    def forward(self, x):
        x = x.view(-1, 1, self.dim) # 250, 1, 99
        # print(f'x shape is{x.shape}')
        x = self.transformer_encoder(x) # 250, 1, 99
        # print(f'x shape is{x.shape}')
        x = x.view(-1, self.dim) # 250, 99
        # print(f'x shape is{x.shape}')
        x = self.fc1(x)  # 250, 8
        # print(f'x shape is{x.shape}')
